...
internal class IntComputerTest {
    ...
    fun `running programs and reading memory0 works as expected`() {
        assertEquals(2, IntComputer("1,0,0,0,99").run().readMemory0())
        assertEquals(30, IntComputer("1,1,1,4,99,5,6,0,99").run().readMemory0())
    }
    ...
    fun `setting noun and verb works as expected`() {
        ...
        assertEquals(12490719, IntComputer(program).setNoun(12).setVerb(2).run().readMemory0())
    }
    ...
    fun `large numbers and relative reading mode are handled`() {
        assertEquals(
            ...
            IntComputer("109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99").run().takeOutputs()
        )
        assertEquals(
            1219070632396864L,
